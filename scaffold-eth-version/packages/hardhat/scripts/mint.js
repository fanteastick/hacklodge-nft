...
const main = async () => {
  ...
  // ADDRESS TO MINT TO:
  const toAddress = "0x4c832889db57FBcb5225736DeaC20BaA452346df"
  ...
  const hacklodge = {
    "description": "From Henry Zhu's presentation on Open-Source and maintaining the Babel compiler",
    "external_url": "https://hacklodge.org/1.0732c06a.png",
    "image": "https://hacklodge.org/1.0732c06a.png",
    "name": "Hack Lodge Photo"
  }

  console.log("Uploading hack lodge photo...")
  const uploaded = await ipfs.add(JSON.stringify(hacklodge))

  console.log("Minting hack lodge photo with IPFS hash ("+uploaded.path+")")
  ...
  const uploadedBuffalo = await ipfs.add(JSON.stringify(buffalo))

  console.log("Minting buffalo with IPFS hash ("+uploadedBuffalo.path+")")
  await yourCollectible.mintItem(toAddress,uploadedBuffalo.path,{gasLimit:400000})
